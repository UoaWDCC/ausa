...
import { ExternalResourceResponse, GetAllExternalResourceResponse } from 'service-layer/response-models/ExternalResourceResponse'
import { ExternalResourceCreationParams, ExternalResourceUpdateParams } from 'data-layer/models/externalResource'
...
export class ExternalResourceController extends Controller {
  @Get()
    public async getAllExternalResources(): Promise<GetAllExternalResourceResponse> {
        try{
            const externalResources = await new ExternalResourceService().getAllExternalResources()
    ...
    @Get('{id}')
    public async getExternalResource(@Path() id: string): Promise<ExternalResourceResponse> {
        try{
    ...
    public async createExternalResource(@Body() newExternalResource: ExternalResourceCreationParams): Promise<ExternalResourceResponse> {
        try {
            const createdExternalResource = await new ExternalResourceService().createExternalResource(newExternalResource)
    ...
    public async updateExternalResource(
        ...
        try {
            ...
            await new ExternalResourceService().updateExternalResource(id, partialExternalResource)
            return {data: await new ExternalResourceService().getExternalResource(id)}
        } catch (error) {
            this.setStatus(500) // Internal Server Error
